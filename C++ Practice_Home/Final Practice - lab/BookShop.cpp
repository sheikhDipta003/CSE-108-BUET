...
class BookShop{
	...
	public:
		...
		/*Initialize with size 5 and name to empty string ""*/
			...
			strcpy(this->name, "");
			this->count = 0;
			this->books = (Book *)malloc(size * sizeof(int));
		}
		...
		/*Write necessary code for the copy constructor*/
			...
			this->size = bs.size;
			this->count += bs.count;
			this->books = (Book*)malloc(this->size * sizeof(int));
		...
		for(int i = 0 ; i < this->count ; i++){
			if(!strcpy(books[i].getTitle(), title)){
				return books[i];
			}
		}			
		}
		...
		for (int i = 0; i < this->count; i++)
		{
			if (books[i].getISBN() == isbn && i != (this->count - 1))
			{
		...
		BookShop mergeShop(BookShop b)
		{
			...
			BookShop newBookShop("MergedShop", this->size + b.size);
			addBook(newBookShop.books, this->count);
			addBook(newBookShop.books, b.count);
		}
};
